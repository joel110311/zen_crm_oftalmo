            ...
            <Table>
                <TableHeader className="bg-slate-50">
                    <TableRow>
                        ...
                        <TableHead className="font-semibold text-slate-600">DURACIÓN</TableHead>
                    ...
                    {appointments.map((apt) => {
                        ...
                        return (
                            <TableRow key={apt.id} className="hover:bg-slate-50/50">
                                ...
                                <TableCell>
                                    <Badge variant={apt.status === 'completed' ? 'default' : 'secondary'} className={
                                        apt.status === 'scheduled' ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200 border-none shadow-none" :
                                            apt.status === 'completed' ? "bg-green-100 text-green-700 hover:bg-green-200 border-none shadow-none" : ""
                                    }>
                                ...
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-blue-600" onClick={() => onEdit(apt)}>
                                            <Edit2 className="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-600 hover:bg-red-50" onClick={() => onDelete(apt.id)}>
